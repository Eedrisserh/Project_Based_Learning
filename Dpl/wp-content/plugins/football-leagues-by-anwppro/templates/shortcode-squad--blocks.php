<?php
/**
 * The Template for displaying Club Squad.
 *
 * This template can be overridden by copying it to yourtheme/anwp-football-leagues/shortcode-squad--blocks.php.
 *
 * @var object $data - Object with shortcode data.
 *
 * @author        Andrei Strekozov <anwp.pro>
 * @package       AnWP-Football-Leagues/Templates
 * @since         0.5.0
 *
 * @version       0.11.13
 */

if ( ! defined( 'ABSPATH' ) ) {
	exit; // Exit if accessed directly
}

// Check required params
if ( empty( $data->club_id ) || empty( $data->season_id ) ) {
	return;
}

// Prevent errors with new params
$data = (object) wp_parse_args(
	$data,
	[
		'season_dropdown'      => 'hide',
		'header'               => true,
		'club_id'              => '',
		'season_id'            => '',
		'layout_block_columns' => '',
	]
);

// Prepare squad
$squad = anwp_football_leagues()->club->tmpl_prepare_club_squad( $data->club_id, $data->season_id, true );

// Prepare staff
$staff = anwp_football_leagues()->club->tmpl_prepare_club_staff( $data->club_id, $data->season_id );

// Initialize staff groups
$staff_group_attached = '';

$default_photo = anwp_football_leagues()->helper->get_default_player_photo();

// Prepare positions
$positions      = anwp_football_leagues()->data->get_positions_plural();
$positions_l10n = [
	'g' => anwp_football_leagues()->get_option_value( 'text_multiple_goalkeeper' ) ?: $positions['g'],
	'd' => anwp_football_leagues()->get_option_value( 'text_multiple_defender' ) ?: $positions['d'],
	'm' => anwp_football_leagues()->get_option_value( 'text_multiple_midfielder' ) ?: $positions['m'],
	'f' => anwp_football_leagues()->get_option_value( 'text_multiple_forward' ) ?: $positions['f'],
];

$data->layout_block_columns = absint( $data->layout_block_columns ) ?: AnWPFL_Options::get_value( 'club_squad_layout_block_columns' );

// Grid columns classes
switch ( $data->layout_block_columns ) {
	case '3':
		$columns_classes = 'anwp-col-lg-4 anwp-col-sm-6 mb-4';
		break;

	case '4':
		$columns_classes = 'anwp-col-lg-4 anwp-col-xl-3 anwp-col-sm-6 mb-4';
		break;

	default:
		$columns_classes = 'anwp-col-sm-6 mb-4';
}
?>
<div class="anwp-b-wrap squad squad--shortcode squad--blocks">

	<?php if ( anwp_football_leagues()->helper->string_to_bool( $data->header ) ) : ?>
		<div class="anwp-block-header__wrapper d-flex justify-content-between">
			<div class="anwp-block-header"><?php echo esc_html( AnWPFL_Text::get_value( 'squad__shortcode__squad', __( 'Squad', 'anwp-football-leagues' ) ) ); ?></div>
			<?php if ( 'hide' !== $data->season_dropdown ) : ?>
				<?php
				$dropdown_filter = [
					'context' => 'club',
					'id'      => $data->club_id,
				];

				anwp_football_leagues()->helper->season_dropdown( $data->season_id, true, '', $dropdown_filter );
				?>
			<?php endif; ?>
		</div>
	<?php endif; ?>

	<?php if ( empty( $squad ) ) : ?>
		<div class="my-3 alert alert-info"><?php echo esc_html( AnWPFL_Text::get_value( 'squad__shortcode__no_players_in_the_squad', __( 'No players in the squad', 'anwp-football-leagues' ) ) ); ?></div>
	<?php else : ?>
		<div class="club__squad">
			<?php foreach ( $positions_l10n as $loop_key => $loop_title ) : ?>
				<div class="anwp-sub-block-header my-2"><?php echo esc_html( $loop_title ); ?></div>
				<div class="anwp-row sm">
					<?php
					foreach ( $squad as $player_id => $player ) :
						if ( $player['position'] !== $loop_key ) {
							continue;
						}

						// Check player status. Do not show players "on trial" or "left"
						if ( in_array( $player['status'], [ 'left', 'on trial' ], true ) ) {
							continue;
						}
						?>
						<div class="<?php echo esc_attr( $columns_classes ); ?>">
							<div class="card position-relative club__player-card h-100 mb-0">
								<div class="card-header anwp-text-center d-flex justify-content-between align-items-center">
									<img class="club__player-photo" src="<?php echo esc_url( $player['photo'] ?: $default_photo ); ?>">
									<div class="club__player-number"><?php echo (int) $player['number'] ? (int) $player['number'] : ''; ?></div>
								</div>
								<div class="card-body club__player-content d-flex flex-column pb-2">

									<?php if ( 'on loan' === $player['status'] ) : ?>
										<span class="badge badge-info club__player-status-badge position-absolute"><?php echo esc_html( AnWPFL_Text::get_value( 'squad__shortcode__on_loan', __( 'On Loan', 'anwp-football-leagues' ) ) ); ?></span>
									<?php endif; ?>

									<h5 class="card-title club__player-name mb-auto pb-3"><?php echo esc_html( $player['name'] ); ?></h5>

									<?php if ( $player['position'] ) : ?>
										<div class="clearfix club__player-param">
											<span class="text-muted small"><?php echo esc_html( AnWPFL_Text::get_value( 'squad__shortcode__position', __( 'Position', 'anwp-football-leagues' ) ) ); ?></span>
											<span class="float-right"><?php echo esc_html( anwp_football_leagues()->data->get_value_by_key( $player['position'], 'position' ) ); ?></span>
										</div>
									<?php endif; ?>

									<?php if ( $player['age'] ) : ?>
										<div class="border-top clearfix club__player-param">
											<span class="text-muted small"><?php echo esc_html( AnWPFL_Text::get_value( 'squad__shortcode__age', __( 'Age', 'anwp-football-leagues' ) ) ); ?></span>
											<span class="float-right"><?php echo esc_html( $player['age'] ); ?></span>
										</div>
									<?php endif; ?>

									<?php if ( ! empty( $player['nationality'] ) && is_array( $player['nationality'] ) ) : ?>
										<div class="border-top clearfix club__player-param">
											<span class="text-muted small d-inline-block pt-1"><?php echo esc_html( AnWPFL_Text::get_value( 'squad__shortcode__nationality', __( 'Nationality', 'anwp-football-leagues' ) ) ); ?></span>
											<span class="float-right">
												<?php foreach ( $player ['nationality'] as $country_code ) : ?>
													<span class="options__flag f32" data-toggle="anwp-tooltip" data-tippy-content="<?php echo esc_attr( anwp_football_leagues()->data->get_value_by_key( $country_code, 'country' ) ); ?>"><span class="flag <?php echo esc_attr( $country_code ); ?>"></span></span>
												<?php endforeach; ?>
											</span>
										</div>
									<?php endif; ?>
								</div>
								<a href="<?php echo esc_url( get_permalink( $player_id ) ); ?>" class="anwp-link-cover club__player-card-link"></a>
							</div>
						</div>
					<?php endforeach; ?>
				</div>
			<?php endforeach; ?>

			<div class="anwp-row">
				<?php
				foreach ( $staff as $staff_id => $staff_member ) :

					if ( 'no' !== $staff_member['grouping'] ) {
						continue;
					}

					if ( $staff_member['job'] !== $staff_group_attached ) :
						?>
						<div class="anwp-col-12">
							<div class="anwp-sub-block-header my-2"><?php echo esc_html( $staff_member['job'] ); ?></div>
						</div>
						<?php $staff_group_attached = $staff_member['job']; ?>
					<?php endif; ?>

					<div class="<?php echo esc_attr( $columns_classes ); ?>">
						<div class="card position-relative club__player-card h-100 mb-0">
							<div class="card-header anwp-text-center d-flex justify-content-between align-items-center">
								<img class="club__player-photo" src="<?php echo esc_url( $staff_member['photo'] ?: $default_photo ); ?>">
							</div>
							<div class="card-body club__player-content d-flex flex-column pb-2">

								<h5 class="card-title club__player-name mb-auto pb-3"><?php echo esc_html( $staff_member['name'] ); ?></h5>

								<?php if ( $staff_member['age'] ) : ?>
									<div class="clearfix club__player-param">
										<span class="text-muted small"><?php echo esc_html( AnWPFL_Text::get_value( 'squad__shortcode__age', __( 'Age', 'anwp-football-leagues' ) ) ); ?></span>
										<span class="float-right"><?php echo esc_html( $staff_member['age'] ); ?></span>
									</div>
								<?php endif; ?>

								<?php if ( ! empty( $staff_member['nationality'] ) && is_array( $staff_member['nationality'] ) ) : ?>
									<div class="border-top clearfix club__player-param">
										<span class="text-muted small d-inline-block pt-1"><?php echo esc_html( AnWPFL_Text::get_value( 'squad__shortcode__nationality', __( 'Nationality', 'anwp-football-leagues' ) ) ); ?></span>
										<span class="float-right">
										<?php foreach ( $staff_member ['nationality'] as $country_code ) : ?>
											<span class="options__flag f32" data-toggle="anwp-tooltip" data-tippy-content="<?php echo esc_attr( anwp_football_leagues()->data->get_value_by_key( $country_code, 'country' ) ); ?>"><span class="flag <?php echo esc_attr( $country_code ); ?>"></span></span>
										<?php endforeach; ?>
									</span>
									</div>
								<?php endif; ?>
							</div>
							<a href="<?php echo esc_url( get_permalink( $staff_id ) ); ?>" class="anwp-link-cover club__player-card-link"></a>
						</div>
					</div>
				<?php endforeach; ?>
			</div>
		</div>

	<?php endif; ?>


	<?php
	$staff_data = [];

	// Prepare staff data
	foreach ( $staff as $staff_id => $staff_member ) :
		if ( 'yes' !== $staff_member['grouping'] ) {
			continue;
		}

		$staff_data[ $staff_member['group'] ][ $staff_id ] = $staff_member;
	endforeach;

	foreach ( $staff_data as $staff_group => $staff_group_items ) :
		$staff_job = '';

		if ( $staff_group ) :
			?>
			<div class="anwp-block-header__wrapper d-flex justify-content-between">
				<div class="anwp-block-header"><?php echo esc_html( $staff_group ); ?></div>
				<?php if ( 'hide' !== $data->season_dropdown ) : ?>
					<?php
					$dropdown_filter = [
						'context' => 'club',
						'id'      => $data->club_id,
					];

					anwp_football_leagues()->helper->season_dropdown( $data->season_id, true, '', $dropdown_filter );
					?>
				<?php endif; ?>
			</div>
		<?php endif; ?>

		<div class="club__squad">
			<div class="anwp-row">
				<?php foreach ( $staff_group_items as $staff_group_item_id => $staff_group_item ) : ?>
					<?php if ( $staff_group_item['job'] !== $staff_job ) : ?>
						<div class="anwp-col-12"><div class="anwp-sub-block-header my-2"><?php echo esc_html( $staff_group_item['job'] ); ?></div></div>
						<?php $staff_job = $staff_group_item['job']; ?>
					<?php endif; ?>

					<div class="<?php echo esc_attr( $columns_classes ); ?>">
						<div class="card position-relative club__player-card h-100 mb-0">
							<div class="card-header anwp-text-center d-flex justify-content-between align-items-center">
								<img class="club__player-photo" src="<?php echo esc_url( $staff_group_item['photo'] ?: $default_photo ); ?>">
							</div>
							<div class="card-body club__player-content d-flex flex-column pb-2">

								<h5 class="card-title club__player-name mb-auto pb-3"><?php echo esc_html( $staff_group_item['name'] ); ?></h5>

								<?php if ( $staff_group_item['age'] ) : ?>
									<div class="clearfix club__player-param">
										<span class="text-muted small"><?php echo esc_html( AnWPFL_Text::get_value( 'squad__shortcode__age', __( 'Age', 'anwp-football-leagues' ) ) ); ?></span>
										<span class="float-right"><?php echo esc_html( $staff_group_item['age'] ); ?></span>
									</div>
								<?php endif; ?>

								<?php if ( ! empty( $staff_group_item['nationality'] ) && is_array( $staff_group_item['nationality'] ) ) : ?>
									<div class="border-top clearfix club__player-param">
										<span class="text-muted small d-inline-block pt-1"><?php echo esc_html( AnWPFL_Text::get_value( 'squad__shortcode__nationality', __( 'Nationality', 'anwp-football-leagues' ) ) ); ?></span>
										<span class="float-right">
										<?php foreach ( $staff_group_item ['nationality'] as $country_code ) : ?>
											<span class="options__flag f32" data-toggle="anwp-tooltip" data-tippy-content="<?php echo esc_attr( anwp_football_leagues()->data->get_value_by_key( $country_code, 'country' ) ); ?>"><span class="flag <?php echo esc_attr( $country_code ); ?>"></span></span>
										<?php endforeach; ?>
									</span>
									</div>
								<?php endif; ?>
							</div>
							<a href="<?php echo esc_url( get_permalink( $staff_group_item_id ) ); ?>" class="anwp-link-cover club__player-card-link"></a>
						</div>
					</div>
				<?php endforeach; ?>
			</div>
		</div>
	<?php endforeach; ?>
</div>
