...
<div class="anwp-b-wrap">
	<table class="align-middle table table-bordered player-list layout-- card-list--<?php echo esc_attr( $data->type ); ?> context--<?php echo esc_attr( $data->context ); ?>">
		...
		<?php foreach ( $items as $index => $p ) : ?>
			<tr class="anwp-text-center">
				...
				<td class="text-left pl-1 <?php echo 'players' === $data->type ? 'py-1' : ''; ?>">
					<?php
					if ( 'players' === $data->type ) :
						...
						<div class="d-flex align-items-center">
							<?php if ( anwp_football_leagues()->helper->string_to_bool( $data->show_photo ) ) : ?>
								<div class="player__photo-wrapper player__photo-wrapper--list anwp-text-center mr-2">
									<img class="player__photo mx-auto" src="<?php echo esc_url( $player->photo ?: $default_photo ); ?>">
								</div>
							<?php endif; ?>

							<div>
								<a class="anwp-link d-block" href="<?php echo esc_url( $player->link ); ?>"><?php echo esc_html( $player->name_short ); ?></a>

								<?php foreach ( $clubs as $ii => $club ) : ?>
									<div class="d-inline-block player-list__club d-flex align-items-center mt-1"><?php echo esc_html( $ii > 0 ? ' | ' : '' ); ?>
										<div class="club-logo__cover club-logo__cover--mini mr-1"
											style="background-image: url('<?php echo esc_url( anwp_football_leagues()->club->get_club_logo_by_id( $club, true ) ); ?>')"></div>
					...
					<?php elseif ( 'clubs' === $data->type ) : ?>
						<div class="d-flex align-items-center">
							...
							<?php endif; ?>
							<a class="club__link anwp-link align-middle" href="<?php echo esc_url( anwp_football_leagues()->club->get_club_link_by_id( $p->club_id ) ); ?>">
